...
from agents.utils.claude import claude
...
result_path = f'/xxx/{model_name}-{test_data_name}'
data_path = f'/xxx/{test_data_name}_benchmark'
...
def predict(image, retries=5, retry_delay=2, index=None):
    for attempt in range(retries):
        try:
            html = claude(prompt_system, image, prompt_user)
...
for index, item in tqdm(enumerate(ds), total=len(ds)):
    ...
    t_start = time.time()
    html = predict(image, index=index) 
